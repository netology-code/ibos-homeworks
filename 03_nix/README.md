...
Настройте запуск сервера при старте системы с рабочим каталогом `/opt/app` (сертификаты должны располагаться в нём же).
...
1. Ответы на впоросы:
    ...
    1. Какие ответы (формат) получает клиент из п.2 получает в ответ на свои запросы
...
Перенастройте сервис из первого ДЗ таким образом, чтобы он запускался от имени пользователя `app` (предварительно создайте его и выставьте необходимые права на файлы и каталоги).

В качестве ответа пришлите скриншот, либо содержимое файла `app.service`.
